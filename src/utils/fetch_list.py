...
DOWNLOAD_DIR = config.get("LIST") or "membership_lists"
...
def fetch_list(
    ...
    today = datetime.now().astimezone().date().strftime("%-m-%-d-%Y")  # matches Periscope's format, e.g. 2-20-2026
    title = f"AllMembers_{today}"
