...
def layout() -> dbc.Row:
    return dbc.Row([dbc.Col(sidebar.sidebar(compare=False), width=2), dbc.Col(membership_map, width=10)], className="dbc", style={"margin": "1em"})


def zoom_level(lat_span: float, lon_span: float) -> int:
    max_span = max(lat_span, lon_span)
    spec = {10: 4, 5: 5, 2: 6, 1: 7, 0.5: 8, 0.2: 9}
    for limit, value in spec.items():
        if max_span > limit:
            return value
    return 10
...
def create_map(date_selected: scan_lists.ISODateStr, selected_column: str, *, is_dark_mode: bool) -> MapFigures:
    ...
    if not df_map.empty and "lat" in df_map.columns and "lon" in df_map.columns:
        ...
        if not df_clean.empty:
            ...
            zoom = zoom_level(lat_span, lon_span)
